        ...
        if user.is_authenticated():
            if not UserProfile.has_registered(user):
                if _can_load_descriptor_nonregistered(descriptor):
                    log.error('Grant direct access to %s', descriptor.category)
                    return ACCESS_GRANTED
                else:
                    log.error('Deny direct access to %s', descriptor.category)
            else:
                log.error('Defer registered access to %s', descriptor.category)
        else:
            log.error('Defer anonymous access to %s', descriptor.category)
        response = (
